var cash_flow_statement = {
    GetCashFlowStatement: function () {
        ...
        if (fromDate == '' || toDate == '') {
            ...
            $.ajax({
                url: localStorage.getItem("baseURL") + '/Accounting/GetCashFlowStatement',
                type: 'POST',
                headers: {
                ...
                success: function (response) {
                    if (response != null) {
                        ...
                        var caption = 'Cash Flow Statement (From ' + captionFromDate + ' To ' + captionToDate + ")";

                        table += '<table id="tbl_cash_flow_statement" class="table table-hover"><caption><b>' + caption + '</b></caption>';
                        ...
                        if (response.length > 0) {
                            ...
                            $("#div_cash_flow_statement").empty().append(table);
                            $("#div_cash_flow_statement").append(totalRecords);
                        }
                        else {
                            $("#div_cash_flow_statement").empty().append(table);
                            $("#div_cash_flow_statement").append('<div class="text-center">No record found</div>');
                        }
                    }
