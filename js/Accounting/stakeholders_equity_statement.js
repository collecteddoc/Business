var stakeholders_equity_statement = {
    GetStakeholdersEquityStatement: function () {
        ...
        if (fromDate == '' || toDate == '') {
            ...
            $.ajax({
                url: localStorage.getItem("baseURL") + '/Accounting/GetStakeholdersEquityStatement',
                type: 'POST',
                headers: {
                ...
                success: function (response) {
                    if (response != null) {
                        ...
                        var caption = 'Stakeholders Equity Statement (From ' + captionFromDate + ' To ' + captionToDate + ")";

                        table += '<table id="tbl_stakeholders_equity_statement" class="table table-hover"><caption><b>' + caption + '</b></caption>';
                        ...
                        if (response.length > 0) {
                            ...
                            $("#div_stakeholders_equity_statement").empty().append(table);
                            $("#div_stakeholders_equity_statement").append(totalRecords);
                        }
                        else {
                            $("#div_stakeholders_equity_statement").empty().append(table);
                            $("#div_stakeholders_equity_statement").append('<div class="text-center">No record found</div>');
                        }
                    }
